...
class TrainerState:
    ...
    best_metric: Optional[float] = float("inf")
...
class LoggerCallback(TrainerCallback):
    def __init__(self, name):
        logging.basicConfig(level=logging.DEBUG,  # 设置日志输出格式
                            # filename="demo.log", #log日志输出的文件位置和文件名,如果设置了就不会输出到控制台
                            # filemode="w", #文件的写入格式，w为重新写入文件，默认是追加
                            format="%(asctime)s - %(levelname)-9s - : %(lineno)s line - %(message)s",
                            datefmt="%Y-%m-%d %H:%M:%S"  # 时间输出的格式
                            )
        self.logger = logging.getLogger(name)

    def on_log(self, logs, level="info"):

        if dist.get_rank() == 0:
            if hasattr(self.logger, level):
                getattr(self.logger, level)(logs)
            else:
                self.logger.warning(f"logger don't have attr {level},we will output the info of logger.")
